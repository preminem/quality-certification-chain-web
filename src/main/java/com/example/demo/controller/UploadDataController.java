...
public class UploadDataController {//数据上传所有功能模块
    ...
    public String certificateUpdate(@RequestBody String param) throws IOException {
        ...
        TransactionDoExample detection = new TransactionDoExample();
        detection.createCriteria().andTemp3EqualTo("5").andTemp1EqualTo(unitID).andTemp2EqualTo(cetificateId);
        List<TransactionDo> list = uploadService.selectByExample(detection);
        //transactionDo.s
        Gson gson = new Gson();
        if (list.isEmpty()) {
            return gson.toJson(R.error("上传认证数据前应该先上传检测数据！"));
        }
